...
//GHL-CRF-0458.1 - end
	...
	public void setManualBarcodeRemarks(HashMap barCodeRemakrs){
		System.out.println("barCodeRemakrs===>"+barCodeRemakrs);
		manualBarcodeRemarks=barCodeRemakrs;
		System.out.println("manualBarcodeRemarks===>"+manualBarcodeRemarks);
	}
...
//System.err.println("=======PARAMS=====locale========>"+locale+" login_facility_id="+login_facility_id+" nursing_unit="+nursing_unit+" patient_class=?"+patient_class+" from_bed_no="+from_bed_no+" to_bed_no="+to_bed_no+" from_time="+from_time+" to_time="+to_time);
	...
	public HashMap getDrugDetails(String encounter_id,String admin_from_date,String admin_to_date,String hold_discontinue_yn,String administration_status,String order_type,String to_time,String from_time, String display_order_by,String route_admin,String admin_rout_categ) throws Exception{
		...
		String OVERRIDE_ADMIN_WIN_DATE_YN		= checkoverrideadminwindow(getAuthUserID());
		
		System.out.println("display_order_by=============>01 "+display_order_by);
		if(display_order_by.equals("D")){
			...
			System.out.println("display_order_by=============>"+display_order_by);
	   ...
	   try{
			connection = getConnection() ;
			 ...
			 boolean  isSite_ph_new_prescription		= eCommon.Common.CommonBean.isSiteSpecific(connection,"PH","PH_NEW_PRESCRIPTION");
			try {
				if(order_type.equals("TPN")){
	...
	//replace 4/24 with ((select nvl(REC_ADMIN_WITHIN_HRS,1) from ph_facility_param where facility_id ='"+login_facility_id+"')/24) in all the below conditions for IN23993 --10/11/2010-- priya
				...
				if(order_type !=null && order_type.equals("Rx")){
					if(!isSite_ph_new_prescription || OVERRIDE_ADMIN_WIN_DATE_YN.equals("N")) ////added By Himanshu Saxena for GHL-CRF-0672
				...
				if(administration_status!=null && (administration_status.equals("OVERDUE") || administration_status.equals("DUE"))){
					appendString = appendString + "AND  ( (A.SCH_DATE_TIME BETWEEN  TO_DATE(?,'DD/MM/YYYY HH24:MI') AND TO_DATE(?,'DD/MM/YYYY HH24:MI') AND (ADMIN_RECORDED_YN='N' OR ADMIN_RECORDED_YN IS NULL)) OR (H.FREQ_NATURE='P'";
					
				
					if(isSite_ph_new_prescription) //added by Himanshu Saxena for against GHL-SCF-1862 
					{
						appendString = appendString +"  AND (ADMIN_RECORDED_YN='N' OR ADMIN_RECORDED_YN IS NULL) and d.start_date_time between  TO_DATE('"+from_time+"','DD/MM/YYYY HH24:MI') AND TO_DATE('"+to_time+"','DD/MM/YYYY HH24:MI') ";
					}else
					{
						appendString = appendString +" and d.start_date_time <  TO_DATE('"+to_time+"','DD/MM/YYYY HH24:MI')  ";
					}
					
					appendString = appendString +" AND (D.ORDER_QTY-NVL(I.TOT_ADMINISTERED_QTY,0))>0 )) "; // added and d.start_date_time <  TO_DATE('"+to_time+"','DD/MM/YYYY HH24:MI') for IN26572 --18/02/2011
				} 
				else if(administration_status!=null && administration_status.equals("ALL")) {
					 appendString = appendString + "AND ( (A.SCH_DATE_TIME BETWEEN  TO_DATE(?,'DD/MM/YYYY HH24:MI') AND TO_DATE(?,'DD/MM/YYYY HH24:MI')) OR (H.FREQ_NATURE='P' ";
					if(isSite_ph_new_prescription) //added by Himanshu Saxena for against GHL-SCF-1862 
					{
						appendString = appendString +" and d.start_date_time between  TO_DATE('"+from_time+"','DD/MM/YYYY HH24:MI') AND TO_DATE('"+to_time+"','DD/MM/YYYY HH24:MI') ";
					}else
					{
						appendString = appendString +" and d.start_date_time <  TO_DATE('"+to_time+"','DD/MM/YYYY HH24:MI')";
					}
					...
					appendString = appendString + "AND  ( (A.SCH_DATE_TIME BETWEEN  TO_DATE(?,'DD/MM/YYYY HH24:MI') AND TO_DATE(?,'DD/MM/YYYY HH24:MI') AND (ADMIN_RECORDED_YN='Y')) OR (H.FREQ_NATURE='P'";
					if(isSite_ph_new_prescription) //added by Himanshu Saxena for against GHL-SCF-1862 
						{
							appendString = appendString +" AND (ADMIN_RECORDED_YN='Y') and d.start_date_time between  TO_DATE('"+from_time+"','DD/MM/YYYY HH24:MI') AND TO_DATE('"+to_time+"','DD/MM/YYYY HH24:MI') ";
						}else
						{
							appendString = appendString +" and d.start_date_time <  TO_DATE('"+to_time+"','DD/MM/YYYY HH24:MI')  ";
						}
	...
	public HashMap getDrugDetails(String encounter_id,String order_id,String shedule_date_time,String drug_code,String hold_discontinue_yn,String administration_status,String order_type, String display_order_by,String route_admin,String admin_rout_categ,String test_patient_id,String empty) throws Exception{   //added String test_patient_id,String empty values for the incident IN037217 and String empty is addded because above we are using one more getDrugDetails method with same number of parameters,so for diffrenciation i added empty String.
		...
		drugCode=drug_code;
		int i=0;
		String cancel_recorded_yn="";// CANCEL_RECORDED_YN ADDED  FOR JD-CRF-0200
        try {
			connection = getConnection() ;
...
//		String time = "";
	...
	public HashMap insert(){
		...
		try{
			...
			setTodaysDate();
			System.out.println("admin_drug_details==> "+admin_drug_details);
			...
			System.out.println("getManualBarcodeRemarks()=>"+getManualBarcodeRemarks());
		...
		try {
		  ...
		  //  pstmt = connection.prepareStatement( PhRepository.getPhKeyValue("SQL_PH_MED_ADMIN_SELECT4")+"and ((b.DISP_QTY>b.RETURNED_QTY) OR (B.RETURNED_QTY IS NULL))" ) ;
			if(administred_yn.equals("N")){
				...
				pstmt = connection.prepareStatement( "SELECT DISTINCT a.BATCH_ID, to_char(a.EXPIRY_DATE,'dd/mm/yyyy') EXPIRY_DATE,d.trade_id,d.short_name trade_name,(SELECT barcode_id FROM st_sal_dtl_exp EXP WHERE EXP.doc_no = e.doc_no AND EXP.doc_srl_no = b.doc_srl_no AND EXP.item_code = a.item_code AND EXP.batch_id = a.batch_id AND EXP.expiry_date_or_receipt_date = a.expiry_date AND EXP.store_code = a.store_code AND EXP.bin_location_code = a.bin_location_code) barcode_id,B.DISP_DRUG_CODE disp_drug_code,a.WARD_ACKNOWLEDGE FROM ph_disp_hdr e,PH_DISP_DRUG_BATCH a, PH_DISP_DTL b,am_trade_name_lang_vw d WHERE e.disp_no=b.disp_no and e.DISP_TYPE!='M'  AND a.facility_id=b.facility_id AND a.disp_no=b.disp_no AND a.srl_no=b.srl_no AND a.trade_id=d.trade_id(+) AND b.ORDER_ID=? AND b.ORDER_LINE_NO=? AND e.order_id = b.order_id  and d.language_id(+) =? and a.disp_qty<>NVL (a.returned_qty, 0)"+"and ((b.DISP_QTY>b.RETURNED_QTY) OR (B.RETURNED_QTY IS NULL))"); //Added AND e.order_id = b.order_id for KAUH10X Issue (No. 73)
			...
			//,a.WARD_ACKNOWLEDGE added for GHL-CRF-0458
				...
				pstmt = connection.prepareStatement( "SELECT DISTINCT a.BATCH_ID, to_char(a.EXPIRY_DATE,'dd/mm/yyyy') EXPIRY_DATE,d.trade_id,d.short_name  trade_name,(SELECT barcode_id FROM st_sal_dtl_exp EXP WHERE EXP.doc_no = e.doc_no AND EXP.doc_srl_no = b.doc_srl_no AND EXP.item_code = a.item_code AND EXP.batch_id = a.batch_id AND EXP.expiry_date_or_receipt_date = a.expiry_date AND EXP.store_code = a.store_code AND EXP.bin_location_code = a.bin_location_code) barcode_id,B.DISP_DRUG_CODE disp_drug_code,a.WARD_ACKNOWLEDGE FROM ph_disp_hdr e, PH_DISP_DRUG_BATCH a, PH_DISP_DTL b,am_trade_name_lang_vw d WHERE e.disp_no=b.disp_no and a.facility_id=b.facility_id AND a.disp_no=b.disp_no AND a.srl_no=b.srl_no AND a.trade_id=d.trade_id(+) AND b.ORDER_ID=? AND b.ORDER_LINE_NO=? AND e.order_id = b.order_id AND d.language_id(+) =? and a.disp_qty<>NVL (a.returned_qty, 0)"+"and ((b.DISP_QTY>b.RETURNED_QTY) OR (B.RETURNED_QTY IS NULL))"); 
				//(SELECT barcode_id FROM st_sal_dtl_exp EXP WHERE EXP.doc_no = e.doc_no AND EXP.item_code = a.item_code AND EXP.batch_id = a.batch_id AND EXP.expiry_date_or_receipt_date = a.expiry_date AND EXP.store_code = a.store_code AND EXP.bin_location_code = a.bin_location_code) barcode_id,B.DISP_DRUG_CODE disp_drug_code added for CRF-413.8
			//,a.WARD_ACKNOWLEDGE added for GHL-CRF-0458				
			}
...
public String validateAckQty(String order_id,String order_line_no,String item_code,String batch_id,String expiry_date,String trade_id,String patient_id,String facility_id,String encounter_id,String admin_dose,String from_time,String to_time){
//added encounter_id for GHL-CRF-0482
		...
		String dosage_type   = "";//GHL-SCF-1878
		float content_in_pres_base_uom	= 0.0f; //GHL-SCF-1878
		float strength_per_pres_uom	= 0.0f; //GHL-SCF-1878
		float total_ack_qty  = 0.0f;
		float tot_admin_qty  = 0.0f; 
		float tot_admin_qty_new  = 0.0f; // GHL-SCF-1878
		float tot_returnable = 0.0f;
		float admin_qty		 = 0.0f; 
		int   cunt			 = 0;//GHL-CRF-0458.1
		float available_stock = 0.0f; 
		ArrayList total_qty;

		try{
			con		= getConnection() ;
			 boolean  isSite_ph_new_prescription		= eCommon.Common.CommonBean.isSiteSpecific(con,"PH","PH_NEW_PRESCRIPTION");
			...
			pstmt   = con.prepareStatement("SELECT a.DRUG_CODE,a.disp_qty total_ack_qty,WARD_ACKNOWLEDGE,c.patient_class,DISP_QTY_UOM, a.disp_qty - (  NVL (a.returned_qty, 0) + NVL ((SELECT SUM (rej_qty) FROM ph_ward_return_dtl WHERE disp_no = a.disp_no  AND srl_no = a.srl_no AND srl_no_batch = a.srl_no_batch and order_id=c.ORDER_ID and facility_id=a.facility_id),0)  + NVL ((SELECT SUM (returned_qty)  FROM ph_ward_return_dtl WHERE disp_no = a.disp_no  AND srl_no = a.srl_no AND srl_no_batch = a.srl_no_batch  and order_id=c.ORDER_ID and facility_id=a.facility_id AND store_acknowledge_status IS NULL),0 ) ) returnable , D.CONTENT_IN_PRES_BASE_UOM , d.STRENGTH_PER_PRES_UOM , E.DOSAGE_TYPE From ph_disp_drug_batch a, ph_disp_dtl b, ph_disp_hdr c , ph_drug_lang_vw d , or_order_line_ph e WHERE a.facility_id = b.facility_id  AND a.disp_no = b.disp_no AND a.srl_no = b.srl_no  AND c.facility_id = b.facility_id AND c.disp_no = b.disp_no  AND c.patient_id =? AND a.facility_id =? AND c.order_id =? AND b.ORDER_LINE_NO=? AND b.order_id = c.order_id  and   a.item_code =? and a.BATCH_ID =? and a.EXPIRY_DATE=to_date(?,'dd/mm/yyyy')  and a.TRADE_ID =? AND E.ORDER_ID=C.ORDER_ID AND E.ORDER_LINE_NUM =b.order_line_no AND a.item_code = D.DRUG_CODE  AND D.LANGUAGE_ID=? ");
			System.err.println("QUERY-:1--> SELECT a.disp_qty total_ack_qty,WARD_ACKNOWLEDGE,c.patient_class,DISP_QTY_UOM, a.disp_qty - (  NVL (a.returned_qty, 0) + NVL ((SELECT SUM (rej_qty) FROM ph_ward_return_dtl WHERE disp_no = a.disp_no  AND srl_no = a.srl_no AND srl_no_batch = a.srl_no_batch and order_id=c.ORDER_ID and facility_id=a.facility_id),0)  + NVL ((SELECT SUM (returned_qty)  FROM ph_ward_return_dtl WHERE disp_no = a.disp_no  AND srl_no = a.srl_no AND srl_no_batch = a.srl_no_batch  and order_id=c.ORDER_ID and facility_id=a.facility_id AND store_acknowledge_status IS NULL),0 ) ) returnable , D.CONTENT_IN_PRES_BASE_UOM , d.strength_value , E.DOSAGE_TYPE From ph_disp_drug_batch a, ph_disp_dtl b, ph_disp_hdr c , ph_drug_lang_vw d , or_order_line_ph e WHERE a.facility_id = b.facility_id  AND a.disp_no = b.disp_no AND a.srl_no = b.srl_no  AND c.facility_id = b.facility_id AND c.disp_no = b.disp_no  AND c.patient_id ='"+patient_id+"' AND a.facility_id ='"+facility_id+"' AND c.order_id ='"+order_id+"' AND b.ORDER_LINE_NO='"+order_line_no+"' AND b.order_id = c.order_id  and   a.item_code ='"+item_code+"' and a.BATCH_ID ='"+batch_id+"' and a.EXPIRY_DATE=to_date('"+expiry_date+"','dd/mm/yyyy')  and a.TRADE_ID ='"+trade_id+"' AND E.ORDER_ID=C.ORDER_ID AND E.ORDER_LINE_NUM =b.order_line_no AND a.item_code = D.DRUG_CODE  AND D.LANGUAGE_ID='"+getLanguageId()+"' ");
		...
		//c.patient_class,DISP_QTY_UOM, added for GHL-CRF-0482
			...
			String sql2			= "SELECT   SUM ((CASE WHEN (SELECT dosage_type FROM or_order_line_ph WHERE order_id = b.order_id AND ROWNUM <= 1) = 'S' THEN CEIL (  ((b.admin_dosage_qty) / d.strength_value) / d.content_in_pres_base_uom ) ELSE CEIL ((b.admin_dosage_qty) / d.content_in_pres_base_uom) END ) ) tot_admin_qty  FROM ph_medn_admin_dtl b,(SELECT drug_desc drug_desc, drug_code drug_code, NVL (content_in_pres_base_uom, 1) content_in_pres_base_uom, strength_value FROM ph_drug_lang_vw WHERE language_id = ? AND drug_code = ?) d WHERE b.encounter_id = ? AND b.ORDER_ID =? AND batch_id = ? AND expiry_date = TO_DATE (?, 'dd/mm/yyyy') AND b.item_code = ? AND b.admin_drug_code = d.drug_code GROUP BY batch_id, expiry_date, trade_id, b.admin_dosage_qty, d.strength_value, d.content_in_pres_base_uom" ;	
			System.err.println("QUERY-:2--> SELECT   SUM ((CASE WHEN (SELECT dosage_type FROM or_order_line_ph WHERE order_id = b.order_id AND ROWNUM <= 1) = 'S' THEN CEIL (  ((b.admin_dosage_qty) / d.strength_value) / d.content_in_pres_base_uom ) ELSE CEIL ((b.admin_dosage_qty) / d.content_in_pres_base_uom) END ) ) tot_admin_qty  FROM ph_medn_admin_dtl b,(SELECT drug_desc drug_desc, drug_code drug_code, NVL (content_in_pres_base_uom, 1) content_in_pres_base_uom, strength_value FROM ph_drug_lang_vw WHERE language_id = '"+getLanguageId()+"' AND drug_code = '"+item_code+"') d WHERE b.encounter_id = '"+encounter_id+"' AND b.ORDER_ID ='"+order_id+"' AND batch_id = '"+batch_id+"' AND expiry_date = TO_DATE ('"+expiry_date+"', 'dd/mm/yyyy') AND b.item_code = '"+item_code+"' AND b.admin_drug_code = d.drug_code GROUP BY batch_id, expiry_date, trade_id, b.admin_dosage_qty, d.strength_value, d.content_in_pres_base_uom");
			...
			while(resultSet!=null && resultSet.next()){
			   ...
			   dosage_type  = resultSet.getString("DOSAGE_TYPE");//GHL-SCF-1878
			   content_in_pres_base_uom  = Float.parseFloat(resultSet.getString("CONTENT_IN_PRES_BASE_UOM"));//GHL-SCF-1878
			   strength_per_pres_uom  = Float.parseFloat(resultSet.getString("STRENGTH_PER_PRES_UOM"));//GHL-SCF-1878
				if(ward_ack_yn.equals("A") || patient_class.equals("OP")){//added for GHL-CRF-0458.1 and patient_class OP added for GHL-CRF-0482
				   ...
				   //added for GHL-SCF-1789 By Himanshu Saxena STARTS
				   if(isSite_ph_new_prescription){
				   String drug_code = resultSet.getString("DRUG_CODE")==null?"":resultSet.getString("DRUG_CODE");
				   total_qty=getTotQty1(patient_id,drug_code,from_time,to_time, order_id, order_line_no);
				   tot_returnable=Float.parseFloat(total_qty.get(1).toString());
				   available_stock=tot_returnable-(Float.parseFloat(total_qty.get(3).toString()));
				   if(dosage_type.equals("S")){ //GHL-SCF-1878
					   available_stock=available_stock*strength_per_pres_uom*content_in_pres_base_uom;
					   tot_returnable=tot_returnable*strength_per_pres_uom*content_in_pres_base_uom;// Added for GHL-SCF-1878
					   tot_admin_qty_new = (Float.parseFloat(total_qty.get(3).toString()))*strength_per_pres_uom*content_in_pres_base_uom; // Added for GHL-SCF-1878
				   }
				   }//added for GHL-SCF-1789 By Himanshu Saxena ENDS
				   else
				   {
...
//			pstmt1.setString(10,trade_id);
			...
			while( resultSet1!=null && resultSet1.next() ) {       //changed if loop to while for GHL-CRF-0482          
				...
				System.err.println(" MedicationAdministrationFTBean.java=====10165=> "+tot_admin_qty);
			}
			if(dosage_type.equals("S")){ //GHL-SCF-1878
				tot_admin_qty = tot_admin_qty_new; 
			}
//System.err.println("MedicationAdministrationFTBean.java====9747==tot_admin_qty===>"+tot_admin_qty);
			closeResultSet( resultSet1 ) ;
			System.out.println("tot_returnable-> "+tot_returnable);
			if(tot_returnable>0){
				tot_returnable = tot_returnable-tot_admin_qty;
			
				System.out.println("tot_admin_qty->  "+tot_admin_qty);
				System.out.println("tot_returnable-> "+tot_returnable);
			...
			//
			if(available_stock<Float.parseFloat(admin_dose))
			{
				allow_admin_yn = "N";
			}
...
//System.err.println("MedicationAdministrationFTBean.java===result===========10175==>"+result);
	...
	public ArrayList getAllBatchIDSForPatientStock(String patient_id,String facility_id,String encounter_id,String drug_code){
		...
		try{
			connection		= getConnection() ;
			boolean  isSite_ph_new_prescription		= eCommon.Common.CommonBean.isSiteSpecific(connection,"PH","PH_NEW_PRESCRIPTION");//added By Himanshu Saxena
...
//System.err.println("PrescriptionBean_1.java===disp_qty=====10430===>"+disp_qty+"==order_id==>"+order_id+"==batch_id==>"+batch_id+"==expiry_date===>"+expiry_date+"==trade_id==>"+trade_id+"==trade_name===>"+trade_name);
				...
				if(isSite_ph_new_prescription){
				ArrayList total_qty=getTotQty1(patient_id,drug_code,"","", order_id, order_line_no);//added By Himanshu Saxena for GHL-SCF-1789 on 
				disp_qty=total_qty.get(1).toString();
				tot_admin_qty=total_qty.get(3).toString();
				barcode_id = "";
...
public String getContent_in_pres_base_uom(String drug_code)
{
	String  base_uom="1";
	Connection connection = null ;
    PreparedStatement pstmt = null ;
    ...
    try {
		connection = getConnection() ;
	    pstmt = connection.prepareStatement("select distinct content_in_pres_base_uom from ph_drug_lang_vw where drug_code = ?" ) ;
		
	    pstmt.setString(1,drug_code);
		resultSet = pstmt.executeQuery() ;
        while ( resultSet != null && resultSet.next() ) {
        	base_uom = checkForNull(resultSet.getString("content_in_pres_base_uom"));
...
public String getfrequencynature(String language,String order_id,String order_line_num)
{
	String  freq_nature="";
	Connection connection = null ;
    PreparedStatement pstmt = null ;
    ...
    try {
		connection = getConnection() ;
	    pstmt = connection.prepareStatement("SELECT (SELECT freq_nature FROM AM_FREQUENCY_LANG_VW WHERE   FREQ_CODE=c.FREQ_CODE and LANGUAGE_ID=?)freq_nature FROM or_order_line_ph a,  or_order b,or_order_line c WHERE a.order_id = ? AND a.order_id = b.order_id and a.order_id = c.order_id and b.order_id = c.order_id  and a.order_line_num = ?  and a.order_line_num = c.order_line_num " ) ;
        ...
        while ( resultSet != null && resultSet.next() ) 
        {
        	freq_nature = checkForNull(resultSet.getString("freq_nature"));
        }
    }
	catch ( Exception e ) {
...
public HashMap cancelAdmisteredDetails(String drug_code,String order_id,String order_line_num,String iv_prep_yn,String sch_date_time,String dosage_seq,String administered_qty,String patient_id,String encounter_id,String admin_date_time,String admin_by_name,String reasonCode,String strength_per_value_pres_uom){//added for jd-crf-0200 start
	...
	System.out.println(" MedicationAdministrationFTBean.java  11074 ===============>cancelAdmisteredDetails called");
	HashMap map = new HashMap() ;
	map.put( "result", new Boolean( false ) ) ;
	map.put( "message", "in insert.." ) ;
	map.put( "flag", "0" ) ;
...
try{
	...
	div_value=1;
	System.out.println("MedicationAdministrationFTBean.java  ===============>dosage_type ===============>dosage_type  "+dosage_type);
	if(dosage_type.equals("S") && !freq_nature.equals("P") ) {
		...
		System.out.println("MedicationAdministrationFTBean.java  ===============>freq_nature ===============>adm_qty  "+freq_nature);
		if(isSite_ph_new_prescription && freq_nature.equals("P"))// added for ghl-scf-1862
			{
			adm_qty	= Float.parseFloat(administered_qty)/Float.parseFloat(getContent_in_pres_base_uom(drug_code));
			System.out.println("MedicationAdministrationFTBean.java  ===============>if condition ===============>adm_qty  "+adm_qty);
			}
		else{
		adm_qty	= Float.parseFloat(administered_qty);
		System.out.println(" MedicationAdministrationFTBean.java  else condition  ===============>adm_qty "+adm_qty);
		}
	}
	
	
	System.out.println("===============> "+Float.parseFloat(administered_qty)*Float.parseFloat(strength_per_value_pres_uom)/div_value);
	System.out.println("===============> "+Float.parseFloat(administered_qty));
	System.out.println("===============> "+getContent_in_pres_base_uom(drug_code));
	System.out.println("===============> "+drug_code);
	
	administered_qty=""+adm_qty;
	
	System.out.println("===============>administered_qty  "+administered_qty);
...
public HashMap cancelAdminstrationForIvRx(String drug_code,String order_id,String order_line_num,String iv_prep_yn,String sch_date_time,String dosage_seq,String administered_qty,String patient_id,String encounter_id,String admin_date_time,String admin_by_name,String reasonCode,String strength_per_value_pres_uom){
	//added for jd-crf-0200 start
		...
		PreparedStatement pstmt4 = null ;
		PreparedStatement pstmt5 = null ;
		boolean updated=true,updated1=true,successFlag1=false;
...
//added for MMS-KH-CRF-0039 - end
	...
	//added By Himanshu Saxena for GHL-CRF-0672 on 22-01-24 Starts
		public String checkoverrideadminwindow(String login_user){
			String OVERRIDE_ADMIN_WIN_DATE_YN	= "";
			Connection connection	= null ;
			PreparedStatement pstmt	= null ;
			ResultSet resultSet		= null ;
			locale = getLanguageId();
			try{
				connection = getConnection() ;
				pstmt			= connection.prepareStatement(PhRepository.getPhKeyValue("SQL_PH_MED_ADMIN_FT_SELECT_OVERRIDE_ADMIN_WIN_DATE")) ;
				pstmt.setString(1,login_user);
				pstmt.setString(2,login_facility_id);
				resultSet			= pstmt.executeQuery();
				if(resultSet!=null && resultSet.next()){
					OVERRIDE_ADMIN_WIN_DATE_YN = resultSet.getString("OVERRIDE_ADMIN_WIN_DATE_YN")==null?"N":resultSet.getString("OVERRIDE_ADMIN_WIN_DATE_YN");
				}
			}
			catch ( Exception e ) {
				e.printStackTrace() ;
			...
			return OVERRIDE_ADMIN_WIN_DATE_YN;
		}
		
		//added By Himanshu Saxena for GHL-SCF-1789 on 
		public ArrayList getTotQty1(String patient_id,String drug_code,String admin_from_date,String admin_to_date,String order_id,String order_line_num)throws Exception{
			
			Connection connection       = null ;
	        PreparedStatement pstmt     = null ;
	        ResultSet resultSet         = null ;
	        String freq_nature=getfrequencynature(locale, order_id, order_line_num);
	        ArrayList totQty					=new ArrayList();	
			String SQL="";
			locale = getLanguageId();
			...
			try{
				...
				if(freq_nature.equals("P"))
				{
				SQL  = " SELECT sum(a.prescribed_qty) prescribed_qty, sum(a.tot_issued_qty) tot_issued_qty, sum(a.returned_qty) returned_qty, sum(a.tot_administered_qty) tot_administered_qty FROM ph_patient_drug_profile a, ph_generic_name_lang_vw b, ph_drug_lang_vw c,am_frequency_lang_vw h,or_order_line o,or_order q,or_order_status_code_lang_vw r,or_order_line_ph s,ph_route k WHERE q.order_id = o.order_id AND a.patient_id =? AND A.START_DATE BETWEEN TRUNC(TO_DATE(NVL (?,'01/01/1472 23:59'), 'DD/MM/YYYY hh24:mi'))  AND TO_DATE(NVL (?,'31/12/5000 23:59'), 'DD/MM/YYYY hh24:mi') + 0.99999 AND a.drug_code BETWEEN NVL (?, '!') AND NVL (?, '~') AND a.generic_id = b.generic_id AND k.route_code = a.route_code AND a.drug_code = c.drug_code AND h.freq_code = a.freq_code  AND a.orig_order_id = o.order_id AND a.orig_order_line_no = o.order_line_num AND o.order_line_status = r.order_status_code  AND o.order_id = s.order_id AND o.order_line_num = s.order_line_num AND b.language_id = c.language_id  AND b.language_id = h.language_id AND b.language_id = r.language_id AND b.language_id = ? AND c.drug_yn = 'Y' and o.order_id='"+order_id+"' and o.order_line_num=?  ";
				}else
				{
					SQL  = " SELECT sum(a.prescribed_qty) prescribed_qty, sum(a.tot_issued_qty) tot_issued_qty, sum(a.returned_qty) returned_qty, sum(a.tot_administered_qty) tot_administered_qty FROM ph_patient_drug_profile a, ph_generic_name_lang_vw b, ph_drug_lang_vw c,am_frequency_lang_vw h,or_order_line o,or_order q,or_order_status_code_lang_vw r,or_order_line_ph s,ph_route k WHERE q.order_id = o.order_id AND a.patient_id =?  AND a.drug_code BETWEEN NVL (?, '!') AND NVL (?, '~') AND a.generic_id = b.generic_id AND k.route_code = a.route_code AND a.drug_code = c.drug_code AND h.freq_code = a.freq_code  AND a.orig_order_id = o.order_id AND a.orig_order_line_no = o.order_line_num AND o.order_line_status = r.order_status_code  AND o.order_id = s.order_id AND o.order_line_num = s.order_line_num AND b.language_id = c.language_id  AND b.language_id = h.language_id AND b.language_id = r.language_id AND b.language_id = ? AND c.drug_yn = 'Y' and o.order_line_num=?  ";
				}
				pstmt			= connection.prepareStatement(SQL) ;
				if(freq_nature.equals("P")){
					pstmt.setString(1,patient_id.trim());
				...
				if(resultSet!=null && resultSet.next()){
					...
					//added for GHL-scf-1862
					if(freq_nature.equals("P")){
					float adm_qty=resultSet.getFloat("TOT_ADMINISTERED_QTY")*Float.parseFloat(getContent_in_pres_base_uom(drug_code.trim()));
					totQty.add(adm_qty);
					}else
					{
						totQty.add(resultSet.getFloat("TOT_ADMINISTERED_QTY"));
		...
		public String Stat_order_id(String routine_order_id){
			...
			try{
			con = getConnection();
			pstmt =   con.prepareStatement("select routine_order_id from or_order_line_ph where  order_id=?");
			pstmt.setString(1, routine_order_id); // Set order_id parameter
			resultSet = pstmt.executeQuery();
			if ( resultSet.next() ) 
			{
			order_id=resultSet.getString("routine_order_id")==null?"":resultSet.getString("routine_order_id");
			}
			}catch (Exception e) 
			{
					e.printStackTrace();
